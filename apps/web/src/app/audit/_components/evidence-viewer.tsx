...
const PdfViewer = dynamic(() => import("./pdf-viewer"), {
  ssr: false,
  loading: () => <p className="p-4 font-mono text-xs text-muted-foreground">loading PDF…</p>,
});
...
function UnitBlock({ unit, citation, active }: { unit: Unit; citation: Citation; active: boolean }) {
  ...
  return (
    <Card ref={ref} className={active ? "ring-1 ring-amber-500/50" : undefined}>
      <CardContent>
        <p className="mb-1.5 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{unit.ref}</p>
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b px-3 py-2">
        <p className="font-mono text-xs text-foreground">{doc.relativePath ?? doc.filename}</p>
        <p className="font-mono text-[10px] text-muted-foreground">{doc.docType ?? doc.kind} · {citation.ref}</p>
        {citation.quote && <Alert className="mt-2 border-amber-500/30 bg-amber-500/10 py-2"><AlertDescription className="font-mono text-[11px]">“{citation.quote}”</AlertDescription></Alert>}
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {doc.kind === "pdf" ? (
          ...
          <p className="p-4 font-mono text-xs text-muted-foreground">loading cited rows…</p>
        ) : packet.table ? (
